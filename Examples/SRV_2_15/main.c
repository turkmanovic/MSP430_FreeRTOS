 ...
 *
 * This task waits for Event "Button S2 Pressed" or "Button S3 Pressed" in Event group.
 * After one of these event is generated by "Button Task", corresponding string is sent
 * over UART to PC
 */
static void prvUARTTaskFunction( void *pvParameters )
{
    EventBits_t eventValue;
    char        string_s2[]  = "Taster S2\r\n";
    char        string_s3[]  = "Taster S3\r\n";
    char        *tmp;
    for ( ;; )
    {
        /* Wait for "Button S2 Pressed" or "Button S3 pressed" event*/
        eventValue = xEventGroupWaitBits(xButtonEventsGroup,
                            mainEVENT_BIT_BUTTON_S2 | mainEVENT_BIT_BUTTON_S3,
                            pdTRUE,
                            pdFALSE,
                            portMAX_DELAY);
        ...
        if(eventValue & mainEVENT_BIT_BUTTON_S2){
            /* If exit is caused by "Button S2 Pressed" event send string_s2*/
            tmp = string_s2;
            while(*tmp != 0){
        ...
        if(eventValue & mainEVENT_BIT_BUTTON_S3){
            /* If exit is caused by "Button S3 Pressed" event send string_s3*/
            tmp = string_s3;
            while(*tmp != 0){
                while (!(UCA1IFG&UCTXIFG));
                UCA1TXBUF   =   *tmp;
