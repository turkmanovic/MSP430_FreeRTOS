 ...
 * @date    2021
 * @brief   SRV Zadatak 12 - Variant 2
 *
 */
...
static void prvString1TaskFunction( void *pvParameters )
{
    ...
    for ( ;; )
    {
        ...
        char* tmp   =   string;
        /* Access to share resource */
        xSemaphoreTake(xGuard_UART, portMAX_DELAY);
        while(*tmp != 0){
            while(!(UCA1IFG&UCTXIFG));
            UCA1TXBUF = *tmp;
            tmp += 1;
        }
        xSemaphoreGive(xGuard_UART);
    }
}
/**
...
void main( void )
{
    ...
    /* Create MUTEX             */
    xGuard_UART             =   xSemaphoreCreateMutex();
    /* Start the scheduler. */
    vTaskStartScheduler();
